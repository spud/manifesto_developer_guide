...
Manifesto takes advantage of an supervisory object called the PageController (instantiated as the object ``$G``), which handles page assembly (sending headers, outputting javascript, responding with 304 Not Modified, etc). It previously served as a monolithic god object, but has been relegated to simpler functionality since Manifesto version 4.
...
* A module that provides the content (stored as ``$request->module``)
* A specific ID for the content (stored as ``$request->id``)
* A specific action to take (edit, display, delete, update, etc, stored as ``$request->function``)
* A specific content type (more common on the backend, stored as ``$request->class``)
...
The ManifestoRouter.php class handles the parsing of URLs to feed into the ManifestoRequest class.

Request
=======
The ManifestoRequest class keeps track of the current request values: the array produced by the parsed URL, the current User, the current Module, and the current Content Object. It is very commonly retrieved within class methods and procedural code to contextualize the flow execution.

Module Prep
===========
Each module has a module_prep file that acts as a sort of module-specific bootstrap. It usually checks access permissions, attempts to retrieve any requested content object (e.g. "the blog post with ID 58") to be stored in $request->contentObj, and loads any module-specific head elements (page title, description). After module_prep, execution passes onto the module's controller file.
...
* Any request for a unique piece of content will require looking up the content in the database, so Manifesto attempts to retrieve the content and stores the result in ``$request->contentObj``. This saves developers the trouble of having to repeat the process of retrieving the content for every method handled by the controller.
...
PageController::addScript('my-custom-javascript', G_URL.'site/themes/my-theme/custom.js');
